...
export default function Client() {
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
      if (editing) {
        ...
        setData((prev) =>
          prev.map((item) =>
            item.clientId === editing.clientId ? res.data : item
          )
        );
  ...
  const handleDelete = async () => {
    try {
      ...
      setData((prev) =>
        prev.filter((item) => item.clientId !== confirmDelete.clientId)
      );
              ...
              {selectedClient?.clientId === item.clientId && (
                <div className="bg-gray-50 px-6 pb-4 pt-2 border-t border-blue-100">
                  <ul className="space-y-1 text-sm text-gray-700">
                    <li>
                      🥛 Sipariş Edilen Süt:{" "}
                      <strong>{item.clientRequestMilk} Litre</strong>
                    </li>
                    <li>
                      ✅ Teslim Edilen Süt:{" "}
                      <strong>{item.clientDeliverMilk} Litre</strong>
                    </li>
                    <li>
                      💵 Tutar: <strong>{item.clientPrice}₺</strong>
                    </li>
                  </ul>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg max-w-md w-full">
            <h3 className="text-lg font-semibold mb-4 text-center text-gray-800">
              {confirmDelete.clientName} {confirmDelete.clientSurname} silinsin
              mi?
            </h3>
